...
/**
 * Loads and parses a JSON configuration file synchronously.
 *
 * This function sends a synchronous GET request to load a JSON file and parses
 * its content into a JavaScript object. If the request fails, it logs an error
 * and returns null.
 *
 * @param {string} filename - The name of the JSON configuration file (without the .json extension).
 * @returns {object|null} - The parsed JSON object if the request is successful; otherwise, null.
 */
function loadConfig(filename) {
    ...
    if (request.status === 200) {
        ...
        console.error('Error loading config data:', request.statusText);
        return null;
    }
}
...
class CVSS40 {
    ...
    static CONFIG = loadConfig('config');
    ...
    initializeVectorMetrics() {
        const selected = {};
        for (let metricType in CVSS40.CONFIG) {
            for (let metricGroup in CVSS40.CONFIG[metricType].metric_groups) {
                for (let metric in CVSS40.CONFIG[metricType].metric_groups[metricGroup]) {
                    selected[CVSS40.CONFIG[metricType].metric_groups[metricGroup][metric].short] = "X";
                }
            }
        }
        return selected;
    ...
    calculateScore() {
        ...
        let value = CVSS40.LOOKUP[this.macroVectorResult]
        ...
        const score_eq1_next_lower_macro = CVSS40.LOOKUP[eq1_next_lower_macro]
        const score_eq2_next_lower_macro = CVSS40.LOOKUP[eq2_next_lower_macro]

        let score_eq3eq6_next_lower_macro;
        if (eq3 == 0 && eq6 == 0) {
            // multiple path take the one with higher score
            const score_eq3eq6_next_lower_macro_left = CVSS40.LOOKUP[eq3eq6_next_lower_macro_left]
            const score_eq3eq6_next_lower_macro_right = CVSS40.LOOKUP[eq3eq6_next_lower_macro_right]

            score_eq3eq6_next_lower_macro = Math.max(score_eq3eq6_next_lower_macro_left, score_eq3eq6_next_lower_macro_right);
        } else {
            score_eq3eq6_next_lower_macro = CVSS40.LOOKUP[eq3eq6_next_lower_macro]
        }


        const score_eq4_next_lower_macro = CVSS40.LOOKUP[eq4_next_lower_macro]
        const score_eq5_next_lower_macro = CVSS40.LOOKUP[eq5_next_lower_macro]
